...
static void _defexcept(struct exception_info *regs);

static interrupt_service_routine_t _handlers[256] = { _defexcept };

static void _halinitgdt(void);
static void _halinitisr(void);

void __dispatch_exception(struct exception_info *regs);
...
void halinit(uint8_t pic1_offset, uint8_t pic2_offset)
{
    ...
    picinit(pic1_offset, pic2_offset);
    // master PIC from offset 0x20-0x27; slave from
    // 0x28-0x2F
}
...
void __dispatch_exception(struct exception_info *regs)
{
    _handlers[regs->vector](regs);
}

void _defexcept(struct exception_info *regs)
{
}
